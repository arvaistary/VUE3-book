...
metadata:
  ...
  source: templates/commands/adopt.md
---
...
3. Fill/update these durable files under `.specify/memory/`:
   ...
   - `architecture/adr/README.md` (human-facing decision index; reference-only — not read by any command step)
   - `development/code-style.md`
   - `ui/screens.md`, `ui/navigation.md`, `ui/conventions.md` (durable UI memory; left as stubs unless the repo already has a UI surface to record — populated over time by `/speckit.finalize` from `ui-plan.md`)
