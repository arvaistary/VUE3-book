...
1. Validate repository readiness:
   ...
   - Require a clean Git worktree before creating a work item. Stop and show
     the existing paths if `git status --porcelain` is not empty.
   - If `.specify/.active-work-item.json` exists, show current active work item
     and ask for confirmation before superseding it. Only after confirmation
     may the script receive `--replace-active`; silent state replacement is
     forbidden.
...
2. Determine mode:
   ...
   - Forward `--replace-active` only after explicit confirmation that the
     previous active item is complete or intentionally abandoned.
...
3. Create the work-item directory and active context by running:
   ...
   - The script records the immutable `base_ref`, `pre_task_status=clean`, and
     the full-mode canonical `spec.md` path for finalization.
...
5. Generation behavior:
   ...
   - **Full mode**: generate/update only `spec.md` in `WORK_ITEM_DIR`; do not auto-generate `plan.md` or `tasks.md`, full STOP. Do not write any code.Do not implement the spec. Output the spec summary and next steps only, then await user input.
     - If a `workflow/project/tasks/TASK-NN-*.md` brief is used, copy its
       contract into `spec.md` before clarify; after clarify, use `spec.md` as
