---
description: Produce and validate an optional UI screen plan (ui-plan.md) for a feature with UI, after planning.
handoffs:
  - label: Create Tasks
    agent: speckit.tasks
...
This command generates and validates `ui-plan.md` for the active work item. It works in
...
1. **Setup**: Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from repo root and parse REPO_ROOT, FEATURE_DIR (folder resolution is the fork's
...
2. **Load context**:
   ...
   - `.specify/memory/constitution.md` (for the Constitution Check, section B). FILE
     ...
     `/speckit.adopt` — as `/speckit.start` does when `.specify/` is absent. A stub file
     (ALL_CAPS placeholders) is NOT a stop: load it, section B yields N/A.
...
6. **Constitution Check**:
   - Section A: run `.specify/scripts/bash/check-ui-plan.sh --json FEATURE_DIR/ui-plan.md`;
...
7. **Completion contract** (Managed Artifact Completion Rules — shared across fork
   commands): re-read the file, remove template markers; if you cannot, STOP, keep the
   partial result and report honestly.
