...
if $PATHS_ONLY; then
    if $JSON_MODE; then
        ...
        printf '{"REPO_ROOT":"%s","SPEC_ROOT":"%s","PRODUCT_ROOT":"%s","PRODUCT_GIT_ROOT":"%s","ARTIFACT_MODE":"%s","BRANCH":"%s","FEATURE_DIR":"%s","FEATURE_SPEC":"%s","IMPL_PLAN":"%s","TASKS":"%s"}\n' \
            "$REPO_ROOT" "$SPEC_ROOT" "$PRODUCT_ROOT" "$PRODUCT_GIT_ROOT" "$ARTIFACT_MODE" "$CURRENT_BRANCH" "$FEATURE_DIR" "$FEATURE_SPEC" "$IMPL_PLAN" "$TASKS"
    else
...
if $JSON_MODE; then
    ...
    printf '{"FEATURE_DIR":"%s","AVAILABLE_DOCS":%s,"SPEC_ROOT":"%s","PRODUCT_ROOT":"%s","PRODUCT_GIT_ROOT":"%s","ARTIFACT_MODE":"%s"}\n' \
        "$FEATURE_DIR" "$json_docs" "$SPEC_ROOT" "$PRODUCT_ROOT" "$PRODUCT_GIT_ROOT" "$ARTIFACT_MODE"
else
    # Text output
    echo "FEATURE_DIR:$FEATURE_DIR"
    echo "SPEC_ROOT:$SPEC_ROOT"
