...
# Get the root implied by this script's installed location.
get_script_repo_root() {
    local script_dir="$(CDPATH="" cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
    (cd "$script_dir/../../.." && pwd -P)
}

read_external_binding_value() {
    local binding_file="$1"
    local key="$2"
    sed -n "s/^[[:space:]]*$key[[:space:]]*=[[:space:]]*\"\(.*\)\"[[:space:]]*$/\1/p" "$binding_file" | head -1
}

get_external_binding_file() {
    local script_root
    script_root="$(get_script_repo_root)"
    echo "$script_root/.specify/external-project.toml"
}

is_external_artifacts_mode() {
    local binding_file
    binding_file="$(get_external_binding_file)"
    [[ -f "$binding_file" ]] && [[ "$(read_external_binding_value "$binding_file" "artifact_mode")" == "external" ]]
}

# Get artifact root, with fallback for non-git repositories.
get_repo_root() {
    local script_root
    script_root="$(get_script_repo_root)"
    if is_external_artifacts_mode; then
        read_external_binding_value "$(get_external_binding_file)" "spec_root"
        return
    fi
...
get_product_root() {
    ...
    binding_file="$(get_external_binding_file)"
    if is_external_artifacts_mode; then
        read_external_binding_value "$binding_file" "product_root"
    else
        get_repo_root
    fi
}

get_product_git_root() {
    local binding_file
    binding_file="$(get_external_binding_file)"
    if is_external_artifacts_mode; then
        read_external_binding_value "$binding_file" "product_git_root"
...
get_current_branch() {
    ...
    if git -C "$product_root" rev-parse --abbrev-ref HEAD >/dev/null 2>&1; then
        git -C "$product_root" rev-parse --abbrev-ref HEAD
        return
    fi
...
has_git() {
    local product_root
    product_root="$(get_product_root)"
    git -C "$product_root" rev-parse --show-toplevel >/dev/null 2>&1
}
