...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --replace-active)
            ...
            ;;
        --help|-h)
            echo "Usage: $0 [--json] [--mode <lite|full>] [--short-name <name>] [--number N] [--replace-active] <description>"
            echo ""
            echo "Options:"
            ...
            echo "  --replace-active    Explicitly supersede the current active work item"
...
WORKFLOW_CORE_DIR="$REPO_ROOT/workflow/core"
if [[ -f "$WORKFLOW_CORE_DIR/artifact-context.sh" ]]; then
    ...
    ARTIFACT_GIT_ROOT="$REPO_ROOT"
    ARTIFACT_PATHSPEC="."
fi
GIT_ROOT="${ARTIFACT_GIT_ROOT:-$REPO_ROOT}"
PATHSPEC="${ARTIFACT_PATHSPEC:-.}"
ACTIVE_STATE_FILE="$REPO_ROOT/.specify/.active-work-item.json"

# A commit ref cannot identify pre-existing untracked files. Require a clean
# worktree so Gate A can attribute every later path to this work item.
if git -C "$GIT_ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    PRE_TASK_STATUS="$(git -C "$GIT_ROOT" status --porcelain --untracked-files=all -- "$PATHSPEC")"
    if [ -n "$PRE_TASK_STATUS" ]; then
        echo "Error: start-work-item requires a clean worktree." >&2
        echo "Commit or preserve the existing changes before starting a new item:" >&2
        printf '%s\n' "$PRE_TASK_STATUS" >&2
        exit 1
    fi
else
    echo "Error: start-work-item requires a Git worktree." >&2
    exit 1
fi

if [[ -f "$ACTIVE_STATE_FILE" && "$REPLACE_ACTIVE" != true ]]; then
    ...
    echo "Error: an active work item already exists: ${ACTIVE_NAME:-unknown} (${ACTIVE_PATH:-unknown})." >&2
    echo "Complete it first or rerun with --replace-active after explicit confirmation." >&2
    exit 1
fi
if [[ -f "$ACTIVE_STATE_FILE" && "$REPLACE_ACTIVE" == true ]]; then
    echo "Warning: explicitly superseding active work item state: $ACTIVE_STATE_FILE" >&2
fi

# Capture the exact pre-work-item commit before creating any Spec-Kit files.
# The Hybrid Finalizer uses this immutable anchor for Gate A.
BASE_REF="$(git -C "$GIT_ROOT" rev-parse --verify HEAD 2>/dev/null || true)"
if [ -z "$BASE_REF" ]; then
    echo "Error: start-work-item requires an initial Git commit for base_ref." >&2
    exit 1
fi
...
CANONICAL_SPEC_PATH=""
if [ "$MODE" = "full" ]; then
    CANONICAL_SPEC_RELATIVE="${RELATIVE_PATH}spec.md"
    CANONICAL_SPEC_PATH="$WORK_ITEM_DIR/spec.md"
fi
...
cat > "$ACTIVE_STATE_FILE" <<EOF
{
  ...
  "base_ref": "$(escape_json "$BASE_REF")",
  "pre_task_status": "clean",
  "canonical_spec": "$(escape_json "$CANONICAL_SPEC_RELATIVE")",
  "created": "$(escape_json "$CREATED_AT")"
}
EOF

if $JSON_MODE; then
    printf '{"WORK_ITEM_NAME":"%s","WORK_ITEM_DIR":"%s","MODE":"%s","BASE_REF":"%s","PRE_TASK_STATUS":"clean","CANONICAL_SPEC":"%s"}\n' \
        "$WORK_ITEM_NAME" "$WORK_ITEM_DIR" "$MODE" "$BASE_REF" "$CANONICAL_SPEC_PATH"
else
    echo "WORK_ITEM_NAME: $WORK_ITEM_NAME"
    echo "WORK_ITEM_DIR: $WORK_ITEM_DIR"
    echo "MODE: $MODE"
    echo "BASE_REF: $BASE_REF"
    echo "PRE_TASK_STATUS: clean"
    echo "CANONICAL_SPEC: $CANONICAL_SPEC_PATH"
