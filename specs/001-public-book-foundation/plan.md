...
Framework-контекст курса фиксируется явно: Nuxt 3 используется как исходная
точка для рассуждений о SSR, состоянии, плагинах, middleware и доставке. В
части 1 добавляется синтетический разбор перехода к структуре Nuxt 4, а
части 2, 3 и 7 проходят отдельный Nuxt-аудит; все фрагменты проверяются по
официальной документации и не описывают закрытый репозиторий.
...
**Testing**: bash workflow/project/scripts/check-book.sh, bash workflow/project/verify-workflow.sh, bash workflow/project/lint-workflow.sh; локальная проверка framework-фрагментов и официальных Nuxt 3/4 ссылок
...
- **Verifiable readiness**: соблюдается для этапа подготовки. Готовность
  ...
  чек-листами; главы уже подготовлены, но финализация выполняется только
  после закрытия оставшихся задач и формирования evidence.
...
6. Для framework-зависимых утверждений сверить версии и примеры миграции,
   состояния, middleware и доставки с официальной документацией Nuxt
   непосредственно перед финализацией.
