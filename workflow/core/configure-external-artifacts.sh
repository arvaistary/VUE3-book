...
# Bind an artifact root to an existing Git worktree for isolated checking.
...
PRODUCT_ROOT="$(CDPATH='' cd "$PRODUCT_ROOT" 2>/dev/null && pwd -P)" || {
    printf 'FAIL: checked root is not an existing directory: %s\n' "$PRODUCT_ROOT" >&2
    exit 1
}
...
PRODUCT_GIT_ROOT="$(CDPATH='' cd "$PRODUCT_GIT_ROOT" 2>/dev/null && pwd -P)" || {
    printf 'FAIL: checked root is not inside a Git worktree: %s\n' "$PRODUCT_ROOT" >&2
    exit 1
}
if [[ "$SPEC_ROOT" == "$PRODUCT_GIT_ROOT" || "$SPEC_ROOT" == "$PRODUCT_GIT_ROOT/"* ]]; then
    printf 'FAIL: artifact root must be outside the checked Git worktree\n' >&2
    exit 1
fi
...
if [[ -f "$BINDING_FILE" && "$OVERWRITE" != true ]]; then
    ...
    if [[ "$EXISTING_PRODUCT" != "$PRODUCT_ROOT" ]]; then
        printf 'FAIL: isolated binding already points to another root; use --overwrite explicitly\n' >&2
        exit 1
    fi
fi
...
printf 'PASS: isolated artifacts binding\n'
